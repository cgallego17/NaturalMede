...
class AuditLogListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = AuditLog.objects.select_related("user", "content_type").order_by(
            "-created_at"
        )
...
def audit_export(request):
    ...
    writer.writerow(
        ["Fecha", "Usuario", "Acción", "Objeto", "Severidad", "Estado", "IP", "Mensaje"]
    )
...
def audit_api(request):
    ...
    if chart_type == "daily":
        ...
        data = list(AuditLog.objects.values("severity").annotate(count=Count("id")))
        return JsonResponse({"data": data})
